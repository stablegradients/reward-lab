...
Pick a starting distribution and press **Play** or **Next step**. Playback runs at a fixed 8× (150 ms per update) for 1,000 updates by default. **Previous step** moves back through recorded updates; going forward replays them before computing another update. **Advanced** holds the algorithm ticks, reward transformations, judge errors, group size, learning rate, seed and run length; every change applies at once. Comparisons show every selected method, and three learning curves sit below them: mean reward and policy entropy over updates, and expected best-of-k reward against k for the displayed update. Hover either step chart to move all three to any recorded update, as in a wandb panel.
