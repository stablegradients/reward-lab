...
This fork trims the visualizer to a comparison of **RLOO, GRPO, TailRL, MaxRL and PKPO** on four starting shapes (bell, uniform, long right tail, binary 5% success) with the Adam optimizer. `core.js` still implements and tests all ten methods; the sound, speed, shape-editor and video-export features of the original are removed.

Pick a starting distribution and press **Play** or **Next step**. One prompt has 21 answers at fixed rewards; each update samples a group of answers from the current policy, scores them, weights the group with the algorithm's rule, and shifts probability toward the better ones. Playback runs at a fixed 8× (150 ms per update) for 1,000 updates by default. **Previous step** moves back through recorded updates; going forward replays them before computing another update. **Advanced** holds the algorithm ticks, reward transformations, judge errors, answers per update, learning rate, seed and run length; every change applies at once. Three learning curves sit below the comparison: mean reward and policy entropy over updates, and expected best-of-k reward against k for the displayed update. Hover either step chart to move all three to any recorded update, as in a wandb panel.
