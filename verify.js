...
// One prompt per update: the plain-SGD step is the mean gradient over the
// group, and methods outside cfg.methods are not simulated.
{
  const s = C.create({
    preset: "bell",
    n: 8,
  ...
  for (const m of ["rloo", "grpo", "pkpo"]) {
    ...
    vector(adv, C.advantage(transformed, m, 4));
  }
  ...
  assert.equal(s.last.tailrl, undefined);
